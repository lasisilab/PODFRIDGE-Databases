...
4. **Save outputs**
   ...
   * `output/foia/foia_state_metadata.csv` — one row per state summarising coverage and caveats.
     After QC passes, I will freeze `foia_data_clean.csv` to `data/v1.0/foia_state_race_2016.csv`.
...
def create_pie_chart(ax, data, offender_type, category, value_type, title, show_values=False):
    ...
    if len(chart_data) > 0:
        ...
        for text in texts:
            text.set_fontsize(8 if category == 'race' else 9)
...
create_state_visualizations(foia_combined, "Nevada")
```
...
indiana['value'] = pd.to_numeric(indiana['value'])

print("✓ Converted Indiana values from String to numeric")
...
Since Indiana only provides percentages for demographics, we need to calculate the actual counts:
